...
export function parseOpenCode2SseEnvelope(payload: string): OpenCode2SseEnvelope | null {
  try {
    ...
    // The daemon beta used {event,data}; current OpenCode emits {id,type,properties}
    // on the same `/api/event` SSE route. Accept both wire shapes.
    if (typeof parsed.event === 'string' && typeof parsed.data === 'string') {
