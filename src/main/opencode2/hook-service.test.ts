...
function currentOpenCodeEnvelope(event: string, properties: unknown): string {
  return `event: message\ndata: ${JSON.stringify({ id: 'evt_1', type: event, properties })}\n\n`
}
...
describe('OpenCode2HookService', () => {
  it('accepts the current OpenCode event envelope and next event names', async () => {
    writeServiceFile()
    vi.stubGlobal(
      'fetch',
      vi.fn(async (_url: string) => ({
        ...
        status: 200,
        body: sseBody([
          currentOpenCodeEnvelope('session.created', {
            sessionID: 'session_1',
            location: { directory: '/repo' }
          }),
          currentOpenCodeEnvelope('session.next.prompted', {
            sessionID: 'session_1',
            prompt: { text: 'inspect the repo' }
          }),
          currentOpenCodeEnvelope('session.next.step.started', { sessionID: 'session_1' }),
          currentOpenCodeEnvelope('session.next.text.ended', {
            sessionID: 'session_1',
    ...
    expect(agentHookServerMock.ingestTerminalStatus).toHaveBeenCalled()
    expect(agentHookServerMock.ingestTerminalStatus.mock.calls.at(-1)![0].payload).toMatchObject({
      state: 'working',
